...
\label{sec:na}
This section enlightens the computational bottlenecks, such as complexity and instabilities, of both the SOB and the HybMMICUB method. The analysis is based on my MATLAB implementations of the two methods
\textit{priceBasketSpreadOption_SecondOrderBoundaryApprox.m} and \textit{priceBasketSpreadOption_HybMMICUB.m}.
\subsection{SOB Method}
There are four remarkable operations in the SOB implementaion. First, there are mainly matrix vector operations which have a computational complexity of $\mathbb(n^2)$ (ignoring scalar operations). Second, in order to compute the values
$mu_{x|y}$ and $\Sigma_{x|y}$ in equation \ref{eq:inv}.
