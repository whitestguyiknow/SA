...
\end{equation}
Additionally we need the correlation coefficient rho between $ln(\mathbb{\tilde{S}}_1)$ and $ln(\mathbb{\tilde{S}}_2)$ to approximate the spread given in equation \ref{eq:spread}. It can be recovered from the eqiality of cross moments (crm):
...
For an approximation of the spread (\ref{eq:spread}) the improved comontonic upper bound (ICUB) is used. The ICUB is displayed in the next proposition, the proof can be found in [2, Appendix]:
\begin{prop}
...
\begin{equation}
\sum_{i=1}^2\epsilon_i\int_0^1e^{\mu_i+A_i(u)+1/2Y_i^2}\Phi\big(Y_i-\Phi^{-1}\big(F_{\mathbb{S}^{ic}|U=u}(K)\big)\big)du-K\big(1-F_{\mathbb{S}^{ic}}(K)\big),
\end{equation}
