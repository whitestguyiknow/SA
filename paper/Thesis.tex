...
Plot (??) shows the two integrands in equation \ref{eq:ints}. Because the curvature of the integrand strongly depends on $u$ I chose to use a (recursive) adaptive Simpson's method as my numerical integration procedure, which is outperforming an integration scheme with equidistant integration intervals. Also the MATLAB routine \textit{integral} is not applicable since we need to find $F_{\mathbb{S}^{ic}}(K)$. The adaptive Simpson's rule repeatedly splits an integral $I = \int_a^b g(x) dx$ in two subintervalls $ I_l = \int_a^{\frac{b-a}{2}} g(x) dx$ and $I_r = \int_{\frac{b-a}{2}}^b g(x) dx$ until the criteria $|I-I_l-I_r| \leq \epsilon$ is met. The integrals are numerically approximated according to the standard Simpson's rule. The accuracy and runtime of the adaptive Simpson's rule strongly depends on the constant $\epsilon$. Based on a performance analysis I set $\epsilon = 10^{-5}$. 
\subsection{Runtime}
\label{rt}
